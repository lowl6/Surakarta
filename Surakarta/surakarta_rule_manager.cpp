...
SurakartaIllegalMoveReason SurakartaRuleManager::JudgeMove(const SurakartaMove& move) {
    ...
    PieceColor move_fromColor = (*board_)[move.from.x][move.from.y]->GetColor();
    PieceColor move_toColor = (*board_)[move.to.x][move.to.y]->GetColor();
    // NOT_PIECE:2
    ...
    if (move_toColor != PieceColor::NONE) {
            ...
            while (1) {
                ...
                if (++i < BOARD_SIZE - 1) {
                    if ((*board_)[i][move.from.y]->GetColor() == PieceColor::NONE)
                        ...
                        break;
                } else if (i == BOARD_SIZE - 1 && (*board_)[i][move.from.y]->GetColor() == PieceColor::NONE) {
                    SurakartaPosition newPosition(i, move.from.y);
                    interface.push_back(newPosition);
                    break;
                } else
            ...
            while (1) {
                ...
                if (--i > 0 && i < BOARD_SIZE) {
                    if ((*board_)[i][move.from.y]->GetColor() == PieceColor::NONE)
                        ...
                        break;
                } else if (i == 0 && (*board_)[i][move.from.y]->GetColor() == PieceColor::NONE) {
                    SurakartaPosition newPosition(i, move.from.y);
                    interface.push_back(newPosition);
                    break;
                } else
        ...
        for (unsigned i = 1; i <= 4; i++) {
            ...
            for (unsigned j = 0; j < BOARD_SIZE; j++) {
                ...
                if ((*board_)[j][i]->GetColor() == PieceColor::NONE)
                    cnt++;
            }
            if (f1)
                cnt = f1 + f2 + cnt;
        ...
        if (!(move.to.y == 0 || move.to.y == BOARD_SIZE - 1)) {
            ...
            while (1) {
                ...
                if (++i < BOARD_SIZE - 1) {
                    if ((*board_)[i][move.to.y]->GetColor() == PieceColor::NONE || (move.from.x == i && move.from.y == move.to.y))
                        continue;
                    else
                        break;
                } else if (i == BOARD_SIZE - 1 && ((*board_)[i][move.to.y]->GetColor() == PieceColor::NONE || (move.from.x == i && move.from.y == move.to.y))) {
                    SurakartaPosition newPosition(i, move.to.y);
                    interface_to.push_back(newPosition);
                    break;
                } else
            ...
            while (1) {
                ...
                if (--i > 0 && i < BOARD_SIZE) {
                    if ((*board_)[i][move.to.y]->GetColor() == PieceColor::NONE || (move.from.x == i && move.from.y == move.to.y))
                        ...
                        break;
                } else if (i == 0 && ((*board_)[i][move.to.y]->GetColor() == PieceColor::NONE || (move.from.x == i && move.from.y == move.to.y))) {
                    SurakartaPosition newPosition(i, move.to.y);
                    interface_to.push_back(newPosition);
                    break;
                } else
        ...
        if (!(move.to.x == 0 || move.to.x == BOARD_SIZE - 1)) {
            ...
            while (1) {
                ...
                if (++i < BOARD_SIZE - 1) {
                    if ((*board_)[move.to.x][i]->GetColor() == PieceColor::NONE || (move.from.x == move.to.x && move.from.y == i))
                        continue;
                    else
                        break;
                } else if (i == BOARD_SIZE - 1 && ((*board_)[move.to.x][i]->GetColor() == PieceColor::NONE || (move.from.x == move.to.x && move.from.y == i))) {
                    SurakartaPosition newPosition(move.to.x, i);
                    interface_to.push_back(newPosition);
                    break;
                } else
...
std::unique_ptr<std::vector<SurakartaPosition>> SurakartaRuleManager::GetAllLegalTarget(const SurakartaPosition postion) {
    ...
    for (unsigned int i = 0; i < current_player_.size(); i++) {
        possible_steps_tmp.from = current_player_[i];
        for (unsigned int j = 0; j < last_player_.size(); j++) {
            possible_steps_tmp.to = last_player_[j];
    ...
    return std::make_unique<std::vector<SurakartaPosition>>(AllLegalTarget);
}
